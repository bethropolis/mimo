...
<section bind:this={sectionEl} class="flex h-full flex-col border border-border/40 bg-surface shadow-sm">
	<div
		class="flex items-center justify-between border-b border-border/40 bg-panel-alt/30 px-3 py-2"
	>
		<div class="flex items-center rounded-xl border border-border/30 bg-surface-muted p-1">
			{#each tabs as pane}
				<button
					type="button"
					onclick={() => (tab = pane.id)}
					class={`relative inline-flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-semibold transition-all duration-200 ${
						tab === pane.id
							? 'z-10 scale-105 bg-accent text-accent-contrast shadow-md'
					...
					<span
						class={`rounded-full px-1.5 py-0.5 text-[10px] ${
							tab === pane.id
								...
								: 'bg-surface-elevated text-text-soft'
						}`}
					>
						{countFor(pane.id)}
					</span>
		...
		<div class="flex items-center gap-2">
			<button
				type="button"
				onclick={copyCurrent}
				class="inline-flex items-center rounded-xl border border-border bg-surface px-3 py-2 text-text-soft transition-colors hover:bg-surface-elevated hover:text-app-fg"
			...
			<button
				...
				class="inline-flex items-center rounded-xl border border-border bg-surface px-3 py-2 text-text-soft transition-colors hover:bg-rose-500/10 hover:text-rose-500"
	...
	<div class="min-h-0 flex-1 overflow-auto scroll-smooth bg-app-bg/30 p-4 font-mono text-sm">
		{#if items.length === 0}
		...
		{:else}
			{#each items as item, index}
				<div
					class={`mb-3 overflow-hidden rounded-xl border shadow-sm transition-all last:mb-0 hover:shadow-md ${tone[tab] ?? tone.output}`}
				>
					<div
						class="flex items-center justify-between border-b border-current/10 bg-current/5 px-3 py-1.5"
					>
					...
					<div class="p-3">
						<p class="leading-relaxed wrap-break-word whitespace-pre-wrap">{item}</p>
					</div>
				</div>
			{/each}
		{/if}
